...
data object Day03 : Task<Int>(3, "Gear Ratios") {
    ...
    private data class SymbolToken(
        val symbol: String,
    ...
    private fun parseInput(lines: List<String>): Pair<Set<NumberToken>, Set<SymbolToken>> {
        ...
        return lines.mapIndexed { y, line ->
            val result = line.foldIndexed(ReductionState()) { x, state, character ->
                when {
                    character.isDigit() -> state.copy(currentNumberToken = state.currentNumberToken?.let {
                        it.copy(
                            value = it.value * 10 + character.digitToInt(), length = it.length + 1
                        )
                    } ?: NumberToken(
                        value = character.digitToInt(), length = 1, origin = Position(x, y)
                    ))

                    character == '.' -> state.copy(
                        numbers = state.numbers + setOfNotNull(state.currentNumberToken), currentNumberToken = null
                    )
